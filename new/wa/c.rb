...

{
  ...
  methods: {
    ...
    # ── Row validation core ───────────────────
    # The body of validate_upload's execute, shared so another action can validate rows
    # against the same model without going through that action's input parsing.
    #   model             parsed canonical model (Hash)
    #   rows              Array of { field_name => value } hashes
    #   variant_field_ids Array of field_id, or nil for all fields
    #   prior_values      Hash keyed by field_id (supplier / engagement scope rules), or nil
    #   options           Hash: max_errors_per_row, max_total_errors, stop_on_first_row_failure, apply_cleaning; or nil
    # Returns the validate_upload output shape: status, summary, errors, valid_payload, validation_result_json.
    validate_rows: lambda do |model, rows, variant_field_ids, prior_values, options|
      fields          = model["cfg_fields"]               || []
      rules           = model["cfg_rules"]                || []
      lookups         = model["cfg_lookups"]              || []
      err_trans       = model["cfg_error_messages"]   || []
      variant_ids     = variant_field_ids                 || []
    ...
    # ── Seed contract helpers ─────────────────
    # A seed file is a multi-supplier submission that has not been split yet: its columns are the
    # canonical model's fields, its rows belong to roster suppliers. These three methods are the
    # whole of the matching vocabulary; validate_seed and describe_seed_contract share them.

    # Column-name key: drop TPL-02's trailing " *" required marker, trim.
    # Mirrors INC-01 / INC-02 _field_key.
    field_key: lambda do |name|
      name.to_s.sub(/\s*\*\s*\z/, "").strip
    end,

    # Lenient key for matching a name VALUE (or a column key): trim, collapse internal whitespace,
    # downcase. Punctuation stays. Mirrors INC-01 _match_key (casefold there; downcase and casefold
    # differ only on a few non-ASCII letters).
    match_key: lambda do |value|
      value.to_s.strip.gsub(/\s+/, " ").downcase
    end,

    # The expected seed columns, derived from the model alone: every cfg_fields entry in position
    # order, with the attributes an analyst needs to author the file. index_key is echoed so the
    # contract is self-describing. Nothing here is stored; it is recomputed from the frozen model.
    seed_contract: lambda do |model, index_key|
      fields = (model["cfg_fields"] || []).sort_by { |f| f["position"].to_i }
      {
        "index_key" => index_key.to_s,
        "columns"   => fields.map do |f|
          {
            "field_name"  => f["field_name"],
            "required"    => call(:coerce_boolean, f["required"]),
            "read_only"   => call(:coerce_boolean, f["read_only"]),
            "data_type"   => f["data_type"],
            "data_format" => f["data_format"].presence,
            "lookup_name" => f["lookup_name"].presence
          }
        end
      }
    end,

    # Apply each field's data_cleaning_flags to one row (the same flags, in the same order, that
    # validate_rows Phase 0 applies). Used to produce the cleaned rows that land in seed slices;
    # validate_rows still cleans its own copy so error messages carry the original value.
    clean_row: lambda do |row, fields|
      cleaned = row.dup
      fields.each do |f|
        next if f["data_cleaning_flags"].blank?
        f["data_cleaning_flags"].split(",").map(&:strip).each do |flag|
          cleaned[f["field_name"]] = call(:apply_cleaning_flag, cleaned[f["field_name"]], flag)
        end
      end
      cleaned
    end,

    # The report as flat lists of strings, for surfaces that cannot render nested objects (a Workflow
    # app page returns scalars and flat lists only). One line per check, status first; one line per
    # detail, prefixed with its check so a single list still reads in order. Same content as checks[].
    seed_report_lines: lambda do |checks|
      check_lines  = []
      detail_lines = []
      (checks || []).each do |c|
        check_lines << "#{c['status'].to_s.upcase}: #{c['check_name']} — #{c['message']}"
        (c["details"] || []).each do |d|
          detail_lines << "#{c['check_name']} › #{d['entity']} '#{d['name']}': #{d['issue']}"
        end
      end
      { "check_lines" => check_lines, "detail_lines" => detail_lines }
    end,

    # The contract as flat lists: the header row the analyst types (field names in template order),
    # and one descriptive line per column. Same content as expected.columns[].
    seed_contract_lines: lambda do |contract|
      columns = contract["columns"] || []
      lines = columns.map do |c|
        parts = [c["data_format"].presence || c["data_type"].presence || "text"]
        parts << "required" if c["required"]
        parts << "read-only (seed only)" if c["read_only"]
        parts << "one of #{c['lookup_name']}" if c["lookup_name"].present?
        "#{c['field_name']} — #{parts.join(', ')}"
      end
      { "expected_header" => columns.map { |c| c["field_name"] }, "expected_lines" => lines }
    end
  },
  ...
  object_definitions: {
    ...
    # ── Seed contract (validate_seed.expected / describe_seed_contract) ──
    seed_contract_definition: {
      fields: lambda do |_connection, _config|
        [
          { name: "index_key",      type: "string",  hint: "Seed column holding the supplier name (customer.incumbent_split_field)." },
          { name: "header_row",     type: "integer", optional: true, hint: "1-based Excel row of the header, as configured on 1_customer. Echoed from the caller." },
          { name: "data_start_row", type: "integer", optional: true, hint: "1-based Excel row where data starts. Echoed from the caller." },
          { name: "sheet_name",     type: "string",  optional: true, hint: "Worksheet to read. Echoed from the caller." },
          { name: "columns",        type: "array",   of: "object", hint: "Every configured field in template position order.",
            properties: [
              { name: "field_name",  type: "string" },
              { name: "required",    type: "boolean" },
              { name: "read_only",   type: "boolean", hint: "Locked in the supplier's template; only a seed can fill it." },
              { name: "data_type",   type: "string" },
              { name: "data_format", type: "string", optional: true },
              { name: "lookup_name", type: "string", optional: true, hint: "Values must be members of this lookup." }
            ] }
        ]
      end
    }
  ...
  actions: {
    ...
    validate_upload: {
      ...
      execute: lambda do |_connection, input, _eis, _eos, _continue|
        ...
        call(:validate_rows, model, rows,
             input["variant_field_ids"], input["prior_values"], input["options"])
      end
    },
    ...
    validate_seed: {
      title: "Validate seed",
      subtitle: "Check a multi-supplier seed file against the canonical model and the supplier roster",
      help: lambda do
        {
          body: "Called by INC-01 after its Python extract step has turned the seed file into rows. Pure computation. " \
                "Matches seed columns to configured fields, resolves the index column against the roster " \
                "(SUP_Supplier joined to SUP_SupplierRequest), validates each supplier's rows with the same engine " \
                "as validate_upload, and returns one report in the validate_config shape, the expected column " \
                "contract, and one slice per supplier request whose rows_json is TPL-02's records_json input."
        }
      end,

      input_fields: lambda do |_object_definitions|
        [
          { name: "seed_rows_json", type: "string", control_type: "text-area", label: "Seed rows (JSON)",
            hint: "JSON array of { header text => value } objects from the INC-01 extract step. Values are already " \
                  "stringified; blank headers were dropped there." },
          { name: "seed_headers", type: "array", of: "string", optional: true,
            hint: "Header cells in column order, from the extract step. When omitted, derived from the rows in first-seen order." },
          { name: "canonical_model_json", type: "string", control_type: "text-area", label: "Canonical model JSON",
            hint: "Content of the project's current template version's canonical model. Read from FileStorage by the caller." },
          { name: "index_key", type: "string",
            hint: "Seed column that holds the supplier name (customer.incumbent_split_field)." },
          { name: "suppliers", type: "array", of: "object", label: "SUP_Supplier rows",
            properties: [
              { name: "supplier_id",   type: "string" },
              { name: "supplier_name", type: "string" }
            ] },
          { name: "supplier_requests", type: "array", of: "object", label: "SUP_SupplierRequest rows (non-terminal states)",
            properties: [
              { name: "supplier_request_id", type: "string" },
              { name: "supplier_id",         type: "string" }
            ] },
          { name: "header_row",     type: "integer", optional: true, hint: "Echoed into expected. Not used for matching; the rows arrive already extracted." },
          { name: "data_start_row", type: "integer", optional: true, hint: "Echoed into expected." },
          { name: "sheet_name",     type: "string",  optional: true, hint: "Echoed into expected." },
          { name: "options", type: "object", optional: true, properties: [
            { name: "validate_values",       type: "boolean", optional: true,
              hint: "Default true. False skips the per-supplier validate_rows pass (structure and roster checks only)." },
            { name: "apply_cleaning",        type: "boolean", optional: true,
              hint: "Default true. Each field's data_cleaning_flags are applied; the cleaned values are what land in slices." },
            { name: "max_rows_per_supplier", type: "integer", optional: true,
              hint: "When set, a slice with more rows fails rows_per_supplier_within_cap. INC-01 passes TPL-02's DATA_ROWS (10000)." }
          ] }
        ]
      end,
      ...
      output_fields: lambda do |object_definitions|
        [
          { name: "status",        type: "string", hint: "valid | invalid. invalid when any check failed." },
          { name: "error_count",   type: "integer" },
          { name: "warning_count", type: "integer" },
          { name: "warnings",      type: "array", of: "object", properties: object_definitions["validation_check"] },
          { name: "checks",        type: "array", of: "object", properties: object_definitions["validation_check"],
            hint: "In order: header_row_holds_index_key, columns_recognized, read_only_fields_supplied, index_values_resolve, " \
                  "rows_per_supplier_within_cap (when a cap is set), values_valid (when validate_values). " \
                  "A failed header or column check ends the report early." },
          { name: "expected",      type: "object", properties: object_definitions["seed_contract_definition"],
            hint: "The seed file spec the analyst reads. Always present, even when the report is invalid." },
          { name: "check_lines",     type: "array", of: "string",
            hint: "One line per check, status first: \"FAIL: columns_recognized — 3 of 11 …\". Same content as checks[], for surfaces that render flat lists only." },
          { name: "detail_lines",    type: "array", of: "string",
            hint: "One line per detail, prefixed with its check: \"columns_recognized › column 'Worker ID': not a configured field name; ignored\"." },
          { name: "expected_header", type: "array", of: "string",
            hint: "The expected header row: field names in template order." },
          { name: "expected_lines",  type: "array", of: "string",
            hint: "One line per expected column: name — type, required, read-only, lookup." },
          { name: "matched",       type: "array", of: "object", hint: "One per distinct seed value that resolved to a request.",
            properties: [
              { name: "supplier_id",         type: "string" },
              { name: "supplier_name",       type: "string" },
              { name: "supplier_request_id", type: "string" },
          ...
          { name: "unmatched",      type: "array", of: "string", hint: "Seed values with no roster supplier. Warned, rows skipped." },
          { name: "unmatched_keys", type: "string", hint: "unmatched joined with ', ' for messages." },
          { name: "ambiguous",      type: "array", of: "object", hint: "Seed values that match more than one request (duplicate supplier names). Fails the report.",
            properties: [
              { name: "seed_value",           type: "string" },
              { name: "supplier_request_ids", type: "array", of: "string" }
            ] },
          { name: "ambiguous_keys", type: "string", hint: "ambiguous seed values joined with ', ' for messages." },
          { name: "counts", type: "object", properties: [
            { name: "rows",             type: "integer", hint: "Seed rows received." },
            { name: "matched",          type: "integer" },
            { name: "unmatched",        type: "integer" },
            { name: "ambiguous",        type: "integer" },
            { name: "slices",           type: "integer", hint: "Distinct requests with rows to seed." },
            { name: "blank_index_rows", type: "integer", hint: "Rows whose index cell is blank. Skipped." }
          ] },
          { name: "slices", type: "array", of: "object", hint: "One per supplier request. Feed rows_json to TPL-02 as records_json.",
            properties: [
              { name: "supplier_request_id", type: "string" },
              { name: "supplier_id",         type: "string" },
              { name: "supplier_name",       type: "string" },
              { name: "seed_values",         type: "array", of: "string", hint: "Every spelling of the name that resolved to this request." },
              { name: "row_count",           type: "integer" },
              { name: "rows_json",           type: "string",
                hint: "JSON array of { field_name => value }: recognized columns only, cleaned when apply_cleaning, original seed order." }
            ] },
          { name: "values_by_supplier", type: "array", of: "object", hint: "Present when validate_values. Per slice, the validate_rows outcome with err_required downgraded to a warning unless the field is read-only.",
            properties: [
              { name: "supplier_request_id", type: "string" },
              { name: "supplier_name",       type: "string" },
              { name: "status",              type: "string", hint: "passed | failed" },
              { name: "summary", type: "object", properties: [
                { name: "total_rows",    type: "integer" },
                { name: "valid_rows",    type: "integer" },
                { name: "invalid_rows",  type: "integer" },
                { name: "error_count",   type: "integer", hint: "Strict findings; these rows fail." },
                { name: "warning_count", type: "integer", hint: "Non-strict findings; reported, rows still seed." }
              ] },
              { name: "errors", type: "array", of: "object", properties: object_definitions["validation_error"] }
            ] }
        ]
      ...
      execute: lambda do |_connection, input, _eis, _eos, _continue|
        # Parse canonical model — defensive against Workato's string-vs-hash pill quirk (as validate_upload)
        raw_model = input["canonical_model_json"]
        model = case raw_model
                when String
                  begin
        ...
        raw_rows = input["seed_rows_json"]
        rows = case raw_rows
               when String
                 begin
                   JSON.parse(raw_rows)
                 rescue JSON::ParserError => e
                   error("Invalid seed_rows_json: #{e.message}")
                 end
               when Array
                 raw_rows
               when NilClass
                 ...
                 error("seed_rows_json is required (JSON string or array).")
               end
        error("seed_rows_json must be an array of objects") unless rows.all? { |r| r.is_a?(Hash) }

        index_key = input["index_key"].to_s.strip
        error("index_key is required") if index_key.empty?

        opts            = input["options"] || {}
        validate_values = opts["validate_values"].nil? ? true : call(:coerce_boolean, opts["validate_values"])
        apply_cleaning  = opts["apply_cleaning"].nil?  ? true : call(:coerce_boolean, opts["apply_cleaning"])
        max_rows        = opts["max_rows_per_supplier"].to_i
        max_rows        = nil if max_rows <= 0

        headers = (input["seed_headers"].presence || rows.flat_map(&:keys).uniq)
                    .map(&:to_s).reject { |h| h.strip.empty? }
        fields  = (model["cfg_fields"] || []).sort_by { |f| f["position"].to_i }

        contract = call(:seed_contract, model, index_key)
        expected = {
          "index_key"      => contract["index_key"],
          "header_row"     => input["header_row"].presence&.to_i,
          "data_start_row" => input["data_start_row"].presence&.to_i,
          "sheet_name"     => input["sheet_name"].presence,
          "columns"        => contract["columns"]
        }

        contract_lines = call(:seed_contract_lines, contract)
        ...
        report = lambda do |extra|
          ...
          warning_count = checks.count { |c| c["status"] == "warn" }
          {
            ...
            "error_count"        => error_count,
            "warning_count"      => warning_count,
            "warnings"           => checks.select { |c| c["status"] == "warn" },
            "checks"             => checks,
            "expected"           => expected,
            "check_lines"        => lines["check_lines"],
            "detail_lines"       => lines["detail_lines"],
            "expected_header"    => contract_lines["expected_header"],
            "expected_lines"     => contract_lines["expected_lines"],
            "matched"            => [],
            "unmatched"          => [],
            "unmatched_keys"     => "",
            "ambiguous"          => [],
            "ambiguous_keys"     => "",
            "counts"             => { "rows" => rows.size, "matched" => 0, "unmatched" => 0, "ambiguous" => 0,
                                      "slices" => 0, "blank_index_rows" => 0 },
            "slices"             => [],
            "values_by_supplier" => []
          }.merge(extra)
        end

        # Column names match case-insensitively, without the " *" marker, and with underscores and spaces
        # alike ("Worker First Name" is worker_first_name). Supplier-name VALUES use match_key unchanged.
        column_key = lambda { |name| call(:match_key, call(:field_key, name)).tr("_", " ") }

        # ── 1. header_row_holds_index_key (structural; stops the run) ──
        index_norm   = column_key.call(index_key)
        index_header = headers.find { |h| column_key.call(h) == index_norm }
        details = []
        if index_header.nil?
          shown = headers.first(12).join(", ") + (headers.size > 12 ? " ..." : "")
          details << { "entity" => "header", "name" => index_key,
                       "issue"  => "not among the #{headers.size} header(s) read: #{shown}" }
        elsif call(:field_key, index_header) != index_key
          details << { "entity" => "header", "name" => index_header,
                       "issue"  => "taken as the index column '#{index_key}' (case or spacing differs)" }
        end
        checks << {
          "check_name" => "header_row_holds_index_key",
          "status"     => index_header ? "pass" : "fail",
          "message"    => index_header ? "Index column '#{index_header}' found on the header row" :
                            "Index column '#{index_key}' is not on the header row",
          "details"    => details
        }
        return report.call({}) if index_header.nil?

        # ── 2. columns_recognized (structural when nothing but the index column matches) ──
        field_by_norm = {}
        fields.each { |f| field_by_norm[column_key.call(f["field_name"])] ||= f }
        column_map = {}      # header text -> field; insertion order = seed column order
        inexact    = []
        duplicate  = []
        unknown    = []
        headers.each do |h|
          key   = call(:field_key, h)
          field = field_by_norm[column_key.call(h)]
          if field.nil?
            unknown << h
          elsif column_map.values.any? { |f| f["field_id"] == field["field_id"] }
            duplicate << [h, field["field_name"]]
          else
            column_map[h] = field
            inexact << [h, field["field_name"]] if key != field["field_name"] && h != index_header   # index noted in check 1
          end
        end
        data_columns = column_map.reject { |h, _| h == index_header }
        details = []
        details.concat(inexact.map   { |h, fn| { "entity" => "column", "name" => h, "issue" => "taken as '#{fn}' (case or spacing differs)" } })
        details.concat(duplicate.map { |h, fn| { "entity" => "column", "name" => h, "issue" => "also maps to '#{fn}'; ignored" } })
        details.concat(unknown.map   { |h|     { "entity" => "column", "name" => h, "issue" => "not a configured field name; ignored" } })
        status = if data_columns.empty? then "fail"
                 elsif unknown.any? || duplicate.any? then "warn"
                 else "pass"
                 end
        checks << {
          "check_name" => "columns_recognized",
          "status"     => status,
          "message"    => if data_columns.empty?
                            "No seed column other than the index matches a configured field name"
                          else
                            "#{column_map.size} of #{headers.size} seed column(s) recognized" +
                              (unknown.any? || duplicate.any? ? "; #{unknown.size + duplicate.size} ignored" : "")
                          end,
          "details"    => details
        }
        return report.call({}) if data_columns.empty?

        active_fields  = column_map.values
        recognized_ids = active_fields.map { |f| f["field_id"] }

        # ── 3. read_only_fields_supplied (warn: only a seed can fill a locked cell) ──
        missing_ro = fields.select { |f| call(:coerce_boolean, f["read_only"]) && !recognized_ids.include?(f["field_id"]) }
        checks << {
          "check_name" => "read_only_fields_supplied",
          "status"     => missing_ro.empty? ? "pass" : "warn",
          "message"    => missing_ro.empty? ? "Every read-only field has a seed column" :
                            "#{missing_ro.size} read-only field(s) have no seed column; the supplier cannot fill them",
          "details"    => missing_ro.map { |f| { "entity" => "field", "name" => f["field_name"],
                                                "issue" => "not in the seed; its cells stay blank and locked" } }
        }

        # ── 4. index_values_resolve (roster: SUP_Supplier joined to SUP_SupplierRequest by supplier_id) ──
        name_by_id = {}
        (input["suppliers"] || []).each do |s|
          sid  = s["supplier_id"].to_s
          name = s["supplier_name"].to_s
          name_by_id[sid] = name if sid.present? && name.present?
        end
        roster_by_norm = {}
        (input["supplier_requests"] || []).each do |r|
          sid = r["supplier_id"].to_s
          rid = r["supplier_request_id"].to_s
          next if sid.empty? || rid.empty?
          name = name_by_id[sid]
          next if name.nil?     # request whose supplier_id has no SUP_Supplier row: unmatchable by name
          (roster_by_norm[call(:match_key, name)] ||= []) << {
            "supplier_id" => sid, "supplier_name" => name, "supplier_request_id" => rid
          }
        end
        ...
        distinct = []
        rows.each do |r|
          value = r[index_header].to_s.strip
          if value.empty?
            blank_index_rows += 1
          elsif !distinct.include?(value)
            distinct << value
          end
        end

        matched, unmatched, ambiguous = [], [], []
        distinct.each do |value|
          candidates = roster_by_norm[call(:match_key, value)] || []
          if candidates.empty?
            unmatched << value
          elsif candidates.size == 1
            matched << candidates.first.merge("seed_value" => value)
          else
            ambiguous << { "seed_value" => value, "supplier_request_ids" => candidates.map { |c| c["supplier_request_id"] } }
          end
        end
        by_request = matched.group_by { |m| m["supplier_request_id"] }
        merged     = by_request.select { |_, ms| ms.size > 1 }

        details = []
        details.concat(unmatched.map { |v| { "entity" => "supplier", "name" => v,
                                             "issue" => "no supplier with an open request has this name; rows skipped" } })
        details.concat(ambiguous.map { |a| { "entity" => "supplier", "name" => a["seed_value"],
                                             "issue" => "matches #{a['supplier_request_ids'].size} requests (#{a['supplier_request_ids'].join(', ')}); " \
                                                        "supplier_name is duplicated in SUP_Supplier" } })
        details.concat(merged.map { |_, ms| { "entity" => "supplier", "name" => ms.first["supplier_name"],
                                              "issue" => "#{ms.size} spellings merged into one slice: #{ms.map { |m| m['seed_value'] }.join(' / ')}" } })
        if blank_index_rows > 0
          details << { "entity" => "row", "name" => "#{blank_index_rows} row(s)", "issue" => "blank '#{index_header}'; skipped" }
        end
        status = if ambiguous.any? || matched.empty? then "fail"
                 elsif unmatched.any? || merged.any? || blank_index_rows > 0 then "warn"
                 else "pass"
                 end
        checks << {
          "check_name" => "index_values_resolve",
          "status"     => status,
          "message"    => if distinct.empty?
                            "No index values in the seed"
                          else
                            "#{matched.size} of #{distinct.size} distinct '#{index_header}' value(s) resolved to #{by_request.size} request(s)" +
                              (unmatched.any? ? "; #{unmatched.size} unmatched" : "") +
                              (ambiguous.any? ? "; #{ambiguous.size} ambiguous" : "")
                          end,
          "details"    => details
        }

        # ── Slices: one per resolved request, rows keyed by field_name, seed order kept ──
        raw_by_request = {}
        slices = by_request.map do |rid, ms|
          values     = ms.map { |m| m["seed_value"] }
          slice_rows = rows.select { |r| values.include?(r[index_header].to_s.strip) }
          records    = slice_rows.map do |r|
            record = {}
            column_map.each { |h, f| record[f["field_name"]] = r[h].to_s }
            record
          end
          raw_by_request[rid] = records
          cleaned = apply_cleaning ? records.map { |rec| call(:clean_row, rec, active_fields) } : records
          {
            "supplier_request_id" => rid,
            "supplier_id"         => ms.first["supplier_id"],
            "supplier_name"       => ms.first["supplier_name"],
            "seed_values"         => values,
            "row_count"           => cleaned.size,
            "rows_json"           => cleaned.to_json
          }
        end

        # ── 5. rows_per_supplier_within_cap (only when a cap was given) ──
        if max_rows
          over = slices.select { |s| s["row_count"] > max_rows }
          checks << {
            "check_name" => "rows_per_supplier_within_cap",
            "status"     => over.empty? ? "pass" : "fail",
            "message"    => over.empty? ? "Every supplier slice is within #{max_rows} rows" :
                              "#{over.size} supplier slice(s) exceed #{max_rows} rows",
            "details"    => over.map { |s| { "entity" => "supplier", "name" => s["supplier_name"],
                                             "issue" => "#{s['row_count']} rows; the template holds #{max_rows}" } }
          }
        end

        # ── 6. values_valid (validate_rows per slice; err_required is a warning unless the field is read-only) ──
        values_by_supplier = []
        if validate_values
          read_only_ids = fields.select { |f| call(:coerce_boolean, f["read_only"]) }.map { |f| f["field_id"] }
          slices.each do |s|
            vr = call(:validate_rows, model, raw_by_request[s["supplier_request_id"]], recognized_ids, nil,
                      { "apply_cleaning" => apply_cleaning })
            errors = vr["errors"].map do |e|
              e = e.dup
              e["strict"] = false if e["error_code"] == "err_required" && !read_only_ids.include?(e["field_id"])
              e
            end
            invalid_rows = errors.select { |e| e["strict"] }.map { |e| e["row_number"] }.uniq
            values_by_supplier << {
              "supplier_request_id" => s["supplier_request_id"],
              "supplier_name"       => s["supplier_name"],
              "status"              => invalid_rows.any? ? "failed" : "passed",
              "summary"             => {
                "total_rows"    => s["row_count"],
                "valid_rows"    => s["row_count"] - invalid_rows.size,
                "invalid_rows"  => invalid_rows.size,
                "error_count"   => errors.count { |e| e["strict"] },
                "warning_count" => errors.count { |e| !e["strict"] }
              },
              "errors"              => errors
            }
          end
          flagged = values_by_supplier.select { |v| v["summary"]["error_count"] > 0 || v["summary"]["warning_count"] > 0 }
          failed  = values_by_supplier.select { |v| v["status"] == "failed" }
          checks << {
            "check_name" => "values_valid",
            "status"     => failed.any? ? "fail" : (flagged.any? ? "warn" : "pass"),
            "message"    => if failed.any?
                              "#{failed.size} of #{values_by_supplier.size} supplier slice(s) have invalid values"
                            elsif flagged.any?
                              "All supplier slices valid; #{flagged.size} with warnings"
                            else
                              "All #{values_by_supplier.size} supplier slice(s) valid"
                            end,
            "details"    => flagged.map { |v|
              sm = v["summary"]
              { "entity" => "supplier", "name" => v["supplier_name"],
                "issue"  => "#{sm['invalid_rows']} of #{sm['total_rows']} row(s) invalid; " \
                            "#{sm['error_count']} error(s), #{sm['warning_count']} warning(s)" }
            }
          }
        end
        ...
        report.call(
          "matched"            => matched,
          "unmatched"          => unmatched,
          "unmatched_keys"     => unmatched.join(", "),
          "ambiguous"          => ambiguous,
          "ambiguous_keys"     => ambiguous.map { |a| a["seed_value"] }.join(", "),
          "counts"             => { "rows" => rows.size, "matched" => matched.size, "unmatched" => unmatched.size,
                                    "ambiguous" => ambiguous.size, "slices" => slices.size,
                                    "blank_index_rows" => blank_index_rows },
          "slices"             => slices,
    ...
    describe_seed_contract: {
      title: "Describe seed contract",
      subtitle: "The expected seed file layout, from the canonical model alone",
      help: lambda do
        {
          body: "For the upload page's hint text before a file exists. Returns the same `expected` object that " \
                "validate_seed reports, derived from the frozen canonical model and the 1_customer seed settings."
        }
      end,
      ...
      input_fields: lambda do |_object_definitions|
        [
          { name: "canonical_model_json", type: "string", control_type: "text-area", label: "Canonical model JSON",
            hint: "Content of the project's current template version's canonical model." },
          { name: "index_key",      type: "string",  hint: "customer.incumbent_split_field." },
          { name: "header_row",     type: "integer", optional: true, hint: "customer.seed_header_row; echoed." },
          { name: "data_start_row", type: "integer", optional: true, hint: "customer.seed_data_start_row; echoed." },
          { name: "sheet_name",     type: "string",  optional: true, hint: "customer.incumbent_sheet_name; echoed." }
        ]
      end,
      ...
      output_fields: lambda do |object_definitions|
        [
          { name: "expected",        type: "object", properties: object_definitions["seed_contract_definition"] },
          { name: "expected_header", type: "array", of: "string", hint: "The expected header row: field names in template order." },
          { name: "expected_lines",  type: "array", of: "string", hint: "One line per expected column: name — type, required, read-only, lookup." }
        ]
      end,
