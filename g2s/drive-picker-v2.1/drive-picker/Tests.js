 ...
 *
 * COVERED: downloadPlan_, isGoogleNative_, withExt_, describeDriveError_, originAllowed_, normalizeOrigin_, parseList_.
...
function runTests() {
  ...
  registerOriginTests_(t);
  registerAppIdTests_(t);
...
// --- originAllowed_ (the helper's host allow-list) ------------------------------------------------
function registerOriginTests_(t) {
  var list = ['https://n-abc123-0lu-script.googleusercontent.com', ' https://n-def456-0lu-script.googleusercontent.com/ '];

  t('origin: exact host matches; trailing slash and whitespace in the list are tolerated', function () {
    assert_(originAllowed_('https://n-abc123-0lu-script.googleusercontent.com', list));
    assert_(originAllowed_('https://n-def456-0lu-script.googleusercontent.com', list));
  });
  t('origin: case-insensitive host', function () {
    assert_(originAllowed_('https://N-ABC123-0lu-script.googleusercontent.com', list));
  });
  t('origin: different host, http, path, or empty are refused', function () {
    assert_(!originAllowed_('https://n-zzz-0lu-script.googleusercontent.com', list));
    assert_(!originAllowed_('http://n-abc123-0lu-script.googleusercontent.com', list));
    assert_(!originAllowed_('https://n-abc123-0lu-script.googleusercontent.com/path', list));
    assert_(!originAllowed_('', list));
    assert_(!originAllowed_('https://n-abc123-0lu-script.googleusercontent.com', []));
  });
  t('normalizeOrigin_ / parseList_', function () {
    assertEq_(normalizeOrigin_('HTTPS://Example.com:8443/'), 'https://example.com:8443');
    assertEq_(normalizeOrigin_('javascript:alert(1)'), '');
    assertEq_(parseList_('a, b\nc,,'), ['a', 'b', 'c']);
    assertEq_(parseList_(null), []);
  });
}

// --- projectNumberFromClientId_ (appId without switching Cloud projects) ------------------------------
function registerAppIdTests_(t) {
  t('appId: project number is the prefix of an OAuth client id', function () {
    assertEq_(projectNumberFromClientId_('123456789012-abc9def.apps.googleusercontent.com'), '123456789012');
    assertEq_(projectNumberFromClientId_('  987654321098-Zz.apps.googleusercontent.com '), '987654321098');
  });
  t('appId: anything else yields empty', function () {
    assertEq_(projectNumberFromClientId_('not-a-client-id'), '');
    assertEq_(projectNumberFromClientId_('12-abc.apps.googleusercontent.com'), '', 'too short to be a project number');
    assertEq_(projectNumberFromClientId_(null), '');
    assertEq_(projectNumberFromClientId_('123456789012-abc.evil.com'), '');
  });
}
