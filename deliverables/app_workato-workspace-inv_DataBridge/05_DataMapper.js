...
class DataMapper {
  ...
  /**
   * Columns 7–8 carry the table's Project and leaf Folder, the same way a recipe row does. Data tables are project-scoped
   * and the API gives a folder_id; the project comes from folderProjectMap ({folder_id: project NAME}, built by the sync
   * from the folders it already holds). Both default to null so older call shapes keep working (the width test calls
   * with folderMap only). The "Folder: X" note in Description stays (for humans reading published docs); the columns are for the code.
   * @param {Array<Object>} dataTables - Raw API response.
   * @param {Object} [folderMap] - Lookup {folder_id: folder name}.
   * @param {Object} [folderProjectMap] - Lookup {folder_id: project name}.
   * @returns {Array<Array<string>>}
   */
  static mapDataTablesToRows(dataTables, folderMap = null, folderProjectMap = null) {
    return (dataTables || []).map(t => {
      ...
      return [
        ...
        String(t.updated_at || ""),
        // Project: an unresolved folder id must not surface here dressed as a project, so this
        // is "-" (the inventory's none marker) rather than _safeLookup's "[ID: x]".
        (t.folder_id && folderProjectMap && folderProjectMap[String(t.folder_id)]) || "-",
        DataMapper._safeLookup(folderMap, t.folder_id)
      ];
    });
  }
