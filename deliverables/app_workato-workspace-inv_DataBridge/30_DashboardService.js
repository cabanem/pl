...
class DashboardService {
  ...
  static _ensureDashboardHome_(ss, ctx, stats) {
    ...
    // ***UPDATED*** O6: the web app is the second front door. The link renders once WEBAPP_URL is
    // stored (menu: Corpus Q&A -> Open web app) and this dashboard is next rebuilt -- absent until
    // then, so it is never a dead link.
    const webAppUrl = ConfigStore.get("WEBAPP_URL", { preferUser: false, defaultValue: "" });
    if (webAppUrl) {
      sh.getRange("D15").setFormula(`=HYPERLINK("${webAppUrl}", "Ask the corpus (web app)")`);
    }
  ...
  static _applyTabColors_(ctx) {
    ...
    const byKey = {
      ...
      DEPENDENCIES: C.analysis, CALL_EDGES: C.analysis, PYTHON_STEPS: C.analysis,
