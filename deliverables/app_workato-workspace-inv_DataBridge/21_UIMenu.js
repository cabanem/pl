...
class UserInterfaceService {
  ...
  createMenu() {
    ...
    if (!isAdv) {
      ...
      root.addSeparator();
      root.addItem("Ask the corpus (Q&A)", "askCorpusPrompt");
      root.addItem("Open Q&A web app", "openCorpusWebApp");   // ***UPDATED*** the team-facing front door
    } else {
      ...
      root.addSubMenu(
        this.ui.createMenu("Python step inventory -> sheet + Drive")
          .addItem("Whole workspace", "extractPythonSteps")
          .addItem("From selection", "extractPythonStepsSelected")
      );
      root.addSubMenu( // ***UPDATED*** Corpus Q&A: interim ask surface + manual digest rebuild
        this.ui.createMenu("Corpus Q&A")
          .addItem("Open web app", "openCorpusWebApp")        // ***UPDATED*** WEBAPP_PLAN §3 / O6
          .addItem("Open Q&A sidebar", "showCorpusSidebar")
          .addItem("Ask a question…", "askCorpusPrompt")
          .addItem("Rebuild corpus digest", "rebuildCorpusDigest")
      );
      ...
      root.addSubMenu(
        this.ui.createMenu("Maintenance")
          ...
          .addItem("Upgrade Q&A log header (one-time)", "migrateQaLogHeaderV2")   // ***UPDATED***
      );
    }
   ...
   */
  showLinkModal(title, message, url, linkLabel) {   // ***UPDATED*** optional label; default keeps existing call sites
    const label = linkLabel || "Open Document in Drive";
