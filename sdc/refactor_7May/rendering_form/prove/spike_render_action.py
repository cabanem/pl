...
# ---------------------------------------------------------------------------
# Spike scaffolding - Workato Python action entry point (Seam 2).
# Paste this whole file into the Python action of the spike GET recipe.
# Output schema: html (string), log (string).
#
# SPIKE SHORTCUT: the fixture is embedded. Before running Seam 4, replace
# DEPLOY_ID in action_url below with your real /exec deployment id so the
...
def main(input):
    ...
    html_out = render(fixture["config"], fixture["values"], fixture["errors"])
    return {"html": html_out, "log": "rendered %d bytes" % len(html_out)}
