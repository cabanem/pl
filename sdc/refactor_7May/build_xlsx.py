...
Sheet geometry (Data):
    ...
    All DV ranges and the cascade INDIRECT anchor are derived from
    DATA_START_ROW so they cannot drift apart.

Structure (two phases, in execution order):
    PLAN  (decide)  — interpret the canonical model into explicit records
                      (ReferenceLayout, [FieldPlan]) with NO openpyxl writes.
                      All branching on data_type / data_format / lookup_name
                      lives here, so field logic has a single home and is
                      testable without building a workbook.
    RENDER (emit)   — dumb writers that walk the records and emit the workbook,
                      making no decisions.
...
# ============================================================================
# 3. ERROR TYPE  — caught only for empty_variant; everything else propagates
# ============================================================================
...
# ============================================================================
# 5. SANITIZATION INVARIANT — load-bearing, do not edit one half alone
# ----------------------------------------------------------------------------
...
# ============================================================================
# 6. RECORD TYPES — the contract between PLAN and RENDER
# ============================================================================
@dataclass
...
# ============================================================================
# 7. PLAN PHASE  (decide — interpret the model into records; no openpyxl writes)
# ============================================================================
...
def _build_advisory_validation(field, cell_range, display):
    ...
    'currency' carries data_type 'string' in the model yet is numeric in practice
...
# ============================================================================
# 8. RENDER PHASE  (emit — walk the records and write the workbook; no decisions)
# ============================================================================
...
def apply_protection(wb, data_ws, ref_ws, plans):
    """
    Lock the template so suppliers edit only intended data-entry cells. Each field
    column's default style is set 'unlocked' unless the plan is locked; empty body
    cells inherit the column default, so editable columns are writable without
    materialising 1k cells/field. Header cells carry their own format and retain
    the default locked=True. Instruction-banner cells are explicitly locked in
    render_data_sheet, so they stay non-editable even in unlocked columns.
    (Formerly _apply_protection, now reading plan.is_locked.)
    """
    for p in plans:
        if p.is_locked:
            continue  # locked col inherits default locked=True
...
# ============================================================================
def main(input):
    # Parse the canonical model. Malformed JSON here is a precondition violation
    # (FileStorage held something invalid) — let it propagate.
    ...
    # ── PLAN (decide): interpret config into records; no openpyxl writes ──
    ...
    # ── RENDER (emit): build the workbook from the records; no decisions ──
