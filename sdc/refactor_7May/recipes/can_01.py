...
Workato entry point is main(input). Everything else is a module-level helper,
one per build phase, so each phase can be reasoned about (and tested) on its own.

Phase order and why it matters:
  1  fields          mint ids, derive control_type, collect the lookup->field map
  1b cascade parents needs the COMPLETE lookup->field map, so it is a second pass
  1c must-fit set   required fields + their cascade ancestors (form-channel viability)
  2  lookups         name-keyed pass-through
  3  rules           resolve field names to ids
  4  variants        mint ids, flatten variant->field membership
  5  form slots      allocate typed slot columns to visible fields
  5b slot cascades   needs ALL slots assigned, so it is a second pass
  6  error messages  pass-through
  6b form channel   viability verdict (viable | degraded | unavailable) from 1c + 5 + 5b
  7  self check      invariants over the assembled collections
"""
...
# SLOT POOL LAYOUT
# ----------------------------------------------------------------------------------------------
# Slot names are LOGICAL join keys, not physical columns: they thread CFG_FormSlotMapping ->
# WFA-018c -> page-variable labels -> the LCAP page's widgets -> WFA-018b/FRM-01 input params ->
# UTL-10 -> RUN_ManualEntry.slot_name. (The slot_* columns on SUP_SupplierRequest are a v0
# vestige and are bound by nothing.) Capacity ground truth is therefore the page's widget
# census and the handler input contracts, which DEFAULT_CAPACITIES mirrors. Capacities arrive
# as input (slot_capacities JSON) so PRV-02 / API-01 / API-02 share one truth; the default
# applies when the input is absent. (slot_sel_11 remains held out for a future multi_select
# control type.)
#
# Allocation policy (form-channel viability): fields are assigned to the first available slot
# whose family matches, in template order. When a family is oversubscribed, must-fit fields
# (required + their cascade ancestors) are allocated first; OPTIONAL fields that do not fit are
# dropped from the form (recorded in fields_without_slots / dropped_optional) -- their data is
# still collectable via the template/upload path. REQUIRED fields that do not fit no longer
# raise: they become per-family deficits and the model reports form_channel_status
# 'unavailable' instead.
DEFAULT_CAPACITIES = {"text": 10, "num": 4, "bool": 2, "sel": 10, "date": 4}

def build_slot_pool(capacities):
    """Derive the named slot pool from per-family capacities."""
    return {family: ["slot_%s_%02d" % (family, n) for n in range(1, count + 1)]
            for family, count in capacities.items()}

# Canonical control_type vocabulary (canonical model shape spec, cfg_fields):
#       text | number | dropdown | dependent_select | date | checkbox | email | currency
# Note:
# - Both select flavours draw from the same `sel` pool, and currency stores as text rather than consuming a `num` slot.
...
# The parser currently emits the depends-on value under `depends_on_field_name` (misleading — it names a LOOKUP, not a field) 
# and may emit `depends_on` once updated. Read all three, prefer the clearest. Single definition so the
# control_type derivation and the cascade resolver can never disagree.
