...
Note on signature evolution (added during Stage 2 design pass). The carry-forward call above settled the actions' capability fit — parsing, validation, report generation, and storage-path construction are still the right concerns for the connector. It did not lock the actions' signatures. The Stage 2 design pass introduced the canonical-model architecture: per-version configuration is fully resolved and persisted to FileStorage at publish time; runtime consumers (PRV-02, VAL-01) read the canonical model directly rather than re-hydrating from CFG_* tables. This shifts validate_upload and validate_config from "take an ID, query data tables" toward "take pre-resolved JSON content, query nothing." The change is non-trivial but well-bounded: the actions' internal validation logic is unchanged; only the input-resolution layer evolves. See sdc-build-queue-v1.md Stage 1 for the expanded connector adjustment list.
