# Workato Data Tables API — Custom Connector v1.6.0b
# ================================================
...

{
  ...
  methods: {
    ...
    table_schema: lambda do |_connection, table_id|
      ...
      resp  = call(:api_call, :get, "/api/data_tables/#{table_id}", nil, "table_schema[#{table_id}]")
  ...
  actions: {
    ...
    get_table: {
      ...
      execute: lambda do |_connection, input|
        resp = call(:api_call, :get, "/api/data_tables/#{input['table_id']}", nil, 'get_table')
    ...
    truncate_table: {
      ...
      execute: lambda do |_connection, input|
        ...
        call(:api_call, :post, "/api/data_tables/#{input['table_id']}/truncate", nil, 'truncate_table')
    ...
    delete_table: {
      ...
      execute: lambda do |_connection, input|
        ...
        call(:api_call, :delete, "/api/data_tables/#{input['table_id']}", nil, 'delete_table')
    ...
    # ── Record CRUD (Records API) ─────────────────────────────────
    # 429 handling is declared, not hand-rolled. The Records API limit
    # is 60 requests/minute across all data table endpoints.

    query_records: {
      title: 'Query records',
      subtitle: 'Search and filter records in a table',
      description: 'Search and filter records in a data table. Returns a ' \
                   'page of results and a continuation token for the next page.',
      ...
      input_fields: lambda do |_object_definitions|
        [
          { name: 'select', type: 'array', of: 'string', optional: true,
            ...
            hint: 'Column names or meta-fields: $record_id, $created_at, $updated_at. ' \
                  'Leave blank to return all columns.' },
          call(:filter_block, 'filters', 'Filters'),
          {
            name: 'order', type: 'object', optional: true, label: 'Sort',
            properties: [
              { name: 'column', type: 'string', hint: 'Column name to sort by.' },
              { name: 'order', control_type: 'select', optional: true, default: 'asc',
                pick_list: [%w[Ascending asc], %w[Descending desc]] },
              { name: 'case_sensitive', type: 'boolean', optional: true, default: 'false' }
            ]
          },
          { name: 'limit', type: 'integer', optional: true, default: '100',
            hint: 'Records per page. Maximum 200.' },
          { name: 'continuation_token', type: 'string', optional: true,
            hint: 'From a previous query response. Leave blank for first page.' },
          { name: 'timezone_offset_secs', type: 'integer', optional: true,
            hint: 'Required when comparing a datetime column to a date value.' }
        ]
      end,

      execute: lambda do |connection, input, _eis, _eos, config_fields|
        result = call(:run_query, connection, config_fields['table_id'],
                      call(:build_where, input['filters']),
                      {
                        'select' => input['select'],
                        'order'  => call(:build_order, input['order']),
                        'limit'  => input['limit'],
                        'continuation_token'   => input['continuation_token'],
                        'timezone_offset_secs' => input['timezone_offset_secs'],
                        'label' => 'query_records'
                      })
        { records: result['records'], continuation_token: result['continuation_token'] }
      end,

      output_fields: lambda do |object_definitions|
        [
          { name: 'records', type: 'array', of: 'object', label: 'Records',
            properties: object_definitions['dynamic_record'] },
          { name: 'continuation_token', type: 'string', label: 'Continuation Token' }
        ]
      end
    },
    ...
    create_record: {
      title: 'Create record',
      subtitle: 'Insert a new record into a table',
      description: 'Insert a new record into a data table. Fields are ' \
                   'dynamically generated from the table schema.',
      ...
      input_fields: lambda do |object_definitions|
        object_definitions['dynamic_record_input']
      end,

      execute: lambda do |connection, input, _eis, _eos, config_fields|
        document = input.reject { |k, _| k == 'table_id' }
        resp = call(:api_call, :post,
                    call(:records_url, connection, config_fields['table_id'], '/records'),
                    { 'payload' => { document: document } }, 'create_record')
        call(:record_from_document, resp)
      end,
    ...
    update_record: {
      title: 'Update record',
      subtitle: 'Update an existing record',
      description: "Update an existing record's field values. Only send " \
                   'the fields you want to change.',

      retry_on_response: [429],
      retry_on_request: ['PUT'],
      max_retries: 3,
      ...
      input_fields: lambda do |object_definitions|
        [
          { name: 'record_id', type: 'string', optional: false, label: 'Record ID',
            hint: 'The $record_id of the record to update.' }
        ] + (object_definitions['dynamic_record_update'] || [])
      end,

      execute: lambda do |connection, input, _eis, _eos, config_fields|
        document = input.reject { |k, _| %w[table_id record_id].include?(k) }
        resp = call(:api_call, :put,
                    call(:records_url, connection, config_fields['table_id'], "/records/#{input['record_id']}"),
                    { 'payload' => { document: document } }, 'update_record')
        call(:record_from_document, resp)
      end,
    ...
    delete_record: {
      title: 'Delete record',
      subtitle: 'Permanently delete a single record',
      ...
      execute: lambda do |connection, input, _eis, _eos, config_fields|
        call(:api_call, :delete,
             call(:records_url, connection, config_fields['table_id'], "/records/#{input['record_id']}"),
             nil, 'delete_record')
        { record_id: input['record_id'], success: true }
      end,

      output_fields: lambda do |_object_definitions|
        [
          { name: 'record_id', type: 'string',  label: 'Deleted Record ID' },
          { name: 'success',   type: 'boolean', label: 'Success' }
        ]
      end
    },

    # ── Batch operations (Records API) ────────────────────────────
    # Per-item failures are captured with after_error_response returning
    # nil (no exception), so one bad row never stops the batch.
    # Bounded by max_batch: the API allows 60 calls/minute.

    batch_create_records: {
      title: 'Batch create records',
      subtitle: 'Create multiple records in a table',
      description: 'Create multiple records in a data table. Each record ' \
                   'is inserted individually; failures are captured without ' \
                   'stopping the batch.',
      ...
      input_fields: lambda do |object_definitions|
        [
          { name: 'records', type: 'array', of: 'object', optional: false,
            label: 'Records', hint: 'List of records to create.',
            properties: object_definitions['dynamic_record_input'] }
        ]
      end,

      execute: lambda do |connection, input, _eis, _eos, config_fields|
        url = call(:records_url, connection, config_fields['table_id'], '/records')
        records = call(:coerce, input['records'], :array)
        error("Batch too large: #{records.length} records. Maximum is 200.") if records.length > 200
        ...
        records.each_with_index do |record, idx|
          failure = nil
          resp = post(url).payload(document: record)
                   .after_error_response(/.*/) do |code, body, headers, _m|
                     failure = { index: idx, record_id: nil, http_code: code.to_i,
                                 message: call(:error_context, "batch_create[#{idx}]", code, body, headers) }
                     nil
                   end
          if failure
            errors << failure
          else
            successes << call(:record_from_document, resp)
          end
        end

        { success_count: successes.length, error_count: errors.length,
          results: successes, errors: errors }
      end,
    ...
    batch_update_records: {
      ...
      subtitle: 'Update multiple records in a table',
      description: 'Update multiple records in a data table. Each record ' \
                   'is updated individually; failures are captured without ' \
                   'stopping the batch.',
      ...
      input_fields: lambda do |object_definitions|
        record_id_field = { name: 'record_id', type: 'string', optional: false,
                            label: 'Record ID', hint: 'The $record_id of the record to update.' }
        [
          { name: 'records', type: 'array', of: 'object', optional: false,
            label: 'Records to update',
            properties: [record_id_field] + (object_definitions['dynamic_record_update'] || []) }
        ]
      end,
      ...
      execute: lambda do |connection, input, _eis, _eos, config_fields|
        ...
        records.each_with_index do |item, idx|
          ...
          document  = item.reject { |k, _| k == 'record_id' }
          url = call(:records_url, connection, config_fields['table_id'], "/records/#{record_id}")
          failure = nil
          resp = put(url).payload(document: document)
                   .after_error_response(/.*/) do |code, body, headers, _m|
                     failure = { index: idx, record_id: record_id, http_code: code.to_i,
                                 message: call(:error_context, "batch_update[#{idx}]", code, body, headers) }
                     nil
                   end
          if failure
            errors << failure
    ...
    batch_delete_records: {
      ...
      subtitle: 'Delete multiple records from a table',
      description: 'Delete multiple records from a data table. Each ' \
                   'delete is processed individually; failures are captured ' \
      ...
      input_fields: lambda do |_object_definitions|
        [
          { name: 'record_ids', type: 'array', of: 'string', optional: false,
            label: 'Record IDs', hint: 'List of $record_id values to delete.' }
        ]
      end,

      execute: lambda do |connection, input, _eis, _eos, config_fields|
        record_ids = call(:coerce, input['record_ids'], :array)
        error("Batch too large: #{record_ids.length} records. Maximum is 200.") if record_ids.length > 200

        successes = []
        errors = []

        record_ids.each_with_index do |record_id, idx|
          url = call(:records_url, connection, config_fields['table_id'], "/records/#{record_id}")
          failure = nil
          delete(url).after_error_response(/.*/) do |code, body, headers, _m|
            failure = { index: idx, record_id: record_id, http_code: code.to_i,
                        message: call(:error_context, "batch_delete[#{idx}]", code, body, headers) }
            nil
          end
          if failure
            errors << failure
    ...
    # ── Workspace management (Developer API) ──────────────────────

    list_folders: {
      title: 'List folders',
      subtitle: 'List workspace folders',
      description: 'Returns folders visible to this API client. Requires ' \
                   'Projects & folders → List scope.',

      input_fields: lambda do |_object_definitions|
        [
          {
            name: 'parent_id', label: 'Parent Folder', optional: true,
            control_type: 'select', pick_list: 'folders',
            toggle_hint: 'Select from list',
            toggle_field: {
              name: 'parent_id', label: 'Parent Folder ID', type: 'string',
              control_type: 'text', optional: true, toggle_hint: 'Enter folder ID'
            },
      ...
      execute: lambda do |_connection, input|
        ...
        params[:parent_id] = call(:coerce, input['parent_id'], :integer) if input['parent_id'].present?

        resp = call(:api_call, :get, '/api/folders', { 'params' => params },
                    'list_folders (check Projects & folders → List scope)')
        { 'data' => call(:normalize_response, resp, :array) }
      end,

      output_fields: lambda do |object_definitions|
        [{ name: 'data', type: 'array', of: 'object', label: 'Folders',
    ...
    list_projects: {
      title: 'List projects',
      subtitle: 'List workspace projects',
      description: 'Returns projects visible to this API client. Requires ' \
                   'Projects & folders → List scope.',
      ...
      execute: lambda do |_connection, input|
        ...
        resp = call(:api_call, :get, '/api/projects',
                    { 'params' => { page: page, per_page: per_page } },
                    'list_projects (check Projects & folders → List scope)')
        { 'data' => call(:normalize_response, resp, :array) }
      end,

      output_fields: lambda do |object_definitions|
        [{ name: 'data', type: 'array', of: 'object', label: 'Projects',
    ...
    create_folder: {
      title: 'Create folder',
      subtitle: 'Create a new workspace folder',
      ...
      execute: lambda do |_connection, input|
        ...
        payload[:parent_id] = call(:coerce, input['parent_id'], :integer) if input['parent_id'].present?

        resp = call(:api_call, :post, '/api/folders', { 'payload' => payload }, 'create_folder')
        call(:normalize_response, resp, :single)
      end,
  ...
  triggers: {
    ...
    new_or_updated_record: {
      ...
      subtitle: 'Triggers on record create or update',
      description: 'Triggers when a record is created or updated in the ' \
                   "selected data table. Set 'Since' to control how far " \
                   'back the first poll looks.',
      ...
      input_fields: lambda do |_object_definitions|
        [
          { name: 'since', type: 'date_time', optional: true, label: 'Since',
            hint: 'Only process records updated after this time. ' \
                  'Defaults to one hour ago if left blank.' },
          { name: 'limit', type: 'integer', optional: true, default: '100',
            hint: 'Records per poll. Maximum 200.' }
        ]
      end,

      poll: lambda do |connection, input, closure, _eis, config_fields|
        since = closure&.dig('since') ||
                (input['since'].present? ? input['since'].to_time.utc.iso8601 : nil) ||
                (now - 1.hour).utc.iso8601
        limit = call(:coerce, input['limit'], :integer) || 100

        # $gte, not $gt: records sharing the cursor timestamp at a page
        # boundary are re-fetched and removed by dedup, never skipped.
        result = call(:run_query, connection, config_fields['table_id'],
                      { '$updated_at' => { '$gte' => since } },
                      { 'order' => { by: '$updated_at', direction: 'asc' },
                        'limit' => limit, 'label' => 'trigger/poll' })
        records = result['records']

        next_since = records.present? ? (records.last['$updated_at'] || since) : since

        {
          events: records,
          next_poll: { 'since' => next_since },
          can_poll_more: records.length >= [limit, 200].min
        }
      end,
