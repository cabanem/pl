...
from extract import extract, PY_PROVIDERS
...
def classify_steps(steps, edges):
    ...
    frames and py_eval are intentionally edge-less (known); anything else that an
    action step leaves behind is UNHANDLED — a relation extract doesn't model yet."""
    edge_uuids = {e.anchor.uuid for e in edges if e.anchor and e.anchor.uuid}
    covered = control = py = 0
...
def run(client, folder_id, scope_name="Recipes", scope_id=None) -> bool:
    ...
    for a in recipes:
        ...
        _cov, _ctrl, _py, unh = classify_steps(steps, edges)
