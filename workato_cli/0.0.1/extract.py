...
# --- candidate input keys (reconcile against the GAS at stage 1) -----------
FLOW_ID_KEYS = ("recipe_id", "flow_id", "recipe")   # callee id may live under any of these
TABLE_KEYS = ("table", "table_id")                  # data-table reference
RECORD_KEYS = ("record", "fields", "data")          # column->value map; keys are column UUIDs

READ_NAMES = {"get_records"}
WRITE_KIND_BY_NAME = {
...
def extract(steps: list[NormStep], source_flow_id: int) -> list[M.Edge]:
    ...
    for s in steps:
        ...
        if s.provider == "py_eval":
            continue                                        # body captured at the step layer; no semantic edge yet

        if s.provider == "workato_recipe_function" and s.name in ("call_recipe", "call_recipe_async"):
            callee = _first(s.input, FLOW_ID_KEYS)
            mode = M.CallMode.async_ if s.name.endswith("_async") else M.CallMode.sync
        ...
        if s.provider == "workato_db_table":
