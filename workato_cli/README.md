...
│                                    #   diff (also holds resolve + projections, inlined)
...
treg = build_table_schema_registry(client.list_data_tables(), fetch_needed_schemas(client, write_tables))
...
Compare the printed facts to the STS-01 rows in your OpenAPI spec. When this is a
routine, it's worth promoting to a `real_oracle.py` with the spec facts baked in as
the assertion set — the same diff `slice_run.py` does, but against live data.
