 ...
 *
 * Determinism: byte counts on canonical JSON (not raw stringify);
 * percentiles via type-7 linear interpolation (numpy/R default);
 * profile hash detects unbumped edits to TRIM_PROFILE.
 *
...
function measureRecipeTrim(recipe, profile) {
  ...
  const before        = canonicalJson(recipe);
  ...
  return {
    ...
    trimmed_hash:       canonicalHash(trimmed),
...
function measureTrimDistribution(recipes, profile) {
  ...
  return {
    profile_version:  profile.version,
    profile_hash:     canonicalHash(profile),
