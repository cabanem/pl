...
class CorpusFixtures {
  ...
  static seededSheets() {
    ...
    // Entity-browser feed: two real tables (one nameless — must fall back to its id) plus a
    // blank-id ghost that must never surface. The digest never reads TABLES, so every existing
    // digest assertion is untouched by this seed.
    // ***UPDATED*** O12 tables: T-20 carries the two appended columns (Project, Folder) and sits
    // in the nested FoldB; T-10 stays SIX wide on purpose — a pre-migration row that must read as
    // "no folder", never break the feed.
    fake.writtenData["TABLES"] = [SchemaDef.HEADERS.TABLES,
    ["T-20", "Suppliers", "Master supplier list | Folder: FoldB", "id,name", "42", iso(3), "ProjA", "FoldB"],
    ["T-10", "", "", "", "", ""],
    ["", "ghost — no id", "", "", "", "", "", ""]];
...
function registerCorpusHermeticTests(runner) {
  ...
  runner.add("[hermetic] doGet serves the Q&A page with all four server calls wired", () => {
    ...
    Assert.contains(html, "entproj", "O12 project level present in the served page");
  });
  ...
  runner.add("[hermetic] getCorpusEntities serves sorted id+name pairs from RECIPES and TABLES through the ledger seam", () => {
    ...
    withLedgerBackend_(fake, () => {
      ...
      // ***UPDATED*** O12 tables: the same shape, from the TABLES row's own columns 6/7.
      Assert.equal(e.tables[0].project, "ProjA", "table project carried from the TABLES row");
      Assert.equal(e.tables[0].folder, "FoldB", "table leaf folder carried from the TABLES row");
      Assert.equal(e.tables[0].path, "ProjA / FoldA / FoldB", "table path resolved through the same resolver");
      Assert.equal(e.tables[1].path, "", "a pre-migration six-wide row has no folder yet — an empty path, not a throw");
    });
  });

  // ***UPDATED*** O12 tables: the contract from mapper to schema to feed. The mapper's columns
  // must land under the headers the feed reads by position — the width test in 50 proves the
  // widths agree; this proves the meaning does.
  runner.add("[hermetic] TABLES rows carry Project and Folder where the schema says and the entity feed reads", () => {
    const h = SchemaDef.HEADERS.TABLES;
    Assert.equal(h.indexOf("Project"), 6, "Project is column 7 — appended, never inserted");
    Assert.equal(h.indexOf("Folder"), 7, "Folder is column 8");
    const rows = DataMapper.mapDataTablesToRows(
      [{ id: 1, name: "T", description: "d", schema: [{ name: "a" }], updated_at: "2024", folder_id: 77 },
       { id: 2, name: "U", description: "", schema: [], updated_at: "", folder_id: 99 },
       { id: 3, name: "V", description: "", schema: [], updated_at: "" }],
      { "77": "FoldB" },
      { "77": "ProjA" });
    Assert.equal(rows[0].length, h.length, "row width matches the eight headers");
    Assert.equal(rows[0][6], "ProjA", "project resolved through the folder");
    Assert.equal(rows[0][7], "FoldB", "leaf folder name");
    Assert.equal(rows[0][2], "d | Folder: FoldB", "the human-readable note in Description stays");
    Assert.equal(rows[1][6], "-", "a folder id the sync cannot place yields the none marker, not an id dressed as a project");
    Assert.equal(rows[1][7], "[ID: 99]", "…while the folder column says honestly what it could not resolve");
    Assert.equal(rows[2][6] + rows[2][7], "--", "no folder id at all -> none, none");
    const legacy = DataMapper.mapDataTablesToRows([{ id: 4, name: "W", schema: [] }], {});
    Assert.equal(legacy[0].length, h.length, "the old two-argument call shape still produces full-width rows");
  });
