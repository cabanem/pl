...
class InventorySyncRunner {
  run(ctx) {
    try {
      ...
      // ***UPDATED*** O12 tables: {folder_id: project NAME}, same idiom as recipeProjectMap — data
      // tables carry a folder_id but no project_id, so their Project column comes through the folder.
      const folderProjectMap = Object.fromEntries(
        (folders || []).map(f => [String(f.id), projectMap[String(f.project_id)] || ""])
      );
      ...
      const dataTableRows = cfg.HEADERS.TABLES && DataMapper.mapDataTablesToRows
        ? [cfg.HEADERS.TABLES, ...DataMapper.mapDataTablesToRows(dataTables, folderMap, folderProjectMap)]   // ***UPDATED*** O12 tables
        : null;
  ...
  run(ctx, idsOverride = null) {
    ...
    ids.forEach((id, idx) => {
      ...
      try {
        ...
        errRow[14] = fpMap.get(String(id)) || '';
        errRow[15] = new Date().toISOString();  
        rows.push(errRow);
      }
...
/** 
 * Gate for fingerprint-driven regeneration. Reads FINGERPRINTS (the ledger's output) and the existing 
 * AI_ANALYSIS rows; splits the estate into {regen, keep}. idsOverride always forces regeneration for those ids —
