...
class SchemaDef {
  ...
  static get HEADERS() {
    return {
      ...
      // ***UPDATED*** O12 tables: Project and Folder appended (never inserted) so nothing that reads
      // this tab by position moves — the same two facts a recipe row carries, so the entity browser
      // resolves a table's folder path exactly the way it resolves a recipe's. Pre-migration rows are
      // six wide and read as "no folder" until the next sync.
      ...
      // ***UPDATED*** Web-app round: Duration ms (the ticket-lane decision reads real p95 from here) and
      // Status ("ok" | "error" — O7: failed asks persist as visible rows). Appended at the END so every existing
      // reader's indexes stay valid; run migrateQaLogHeaderV2 once to  widen a live tab's header
  ...
  static get() {
    return {
      ...
      QA: {
        ...
        // ***UPDATED*** Starter chips for the web app; status() caps what it serves at six. The four below
        // are block-shaped (they work on any estate) — curate toward the estate's own names as they earn it.
        STARTER_QUESTIONS: [
          "Which recipes call each other, and where are the weak or ambiguous references?",
          "What changed in the estate in the last two weeks?",
