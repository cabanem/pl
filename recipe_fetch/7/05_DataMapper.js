...
class DataMapper {
  ...
  /**
   * ***UPDATED*** O12 tables: columns 7–8 carry the table's Project and leaf Folder, the same way
   * a recipe row does. Data tables are project-scoped and the API gives a folder_id; the project
   * comes from folderProjectMap ({folder_id: project NAME}, built by the sync from the folders it
   * already holds). Both default to null so older call shapes keep working (the width test calls
   * with folderMap only). The "Folder: X" note in Description stays — it's for the humans reading
   * published docs; the columns are for the code.
