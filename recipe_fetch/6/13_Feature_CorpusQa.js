 ...
 *
 *   ***UPDATED*** O12 (entity-browser display evolution): entities() now carries each recipe's
 *   project, leaf folder, and a full folder path resolved through Inventory_Folders by the pure
 *   folderPathResolver_ (name-joined, so ambiguity and cycles fall back to "project / leaf" rather
 *   than guess). Also the never-throws fix in status(): its degradation log no longer depends on
 *   AppConfig, which was the one dependency that could make "never throws" false.
 *
...
class CorpusQaService {
   ...
   * rows dropped, a nameless row falls back to its id. Read-only, no new scopes, no schema change.
   *
   * ***UPDATED*** O12: recipes also carry their project, leaf folder (both straight from the
   * recipe row) and a full folder PATH resolved through Inventory_Folders — one extra read on the
   * same seam, resolved by folderPathResolver_. The client groups the Recipes section by that
   * path. Tables stay id+name: the only folder fact they carry is a free-text note in their
   * description, which is not a join key.
   *
  ...
  static entities() {
    ...
    const nameOf = (r, id) => CorpusQaService.cell_(r[1]) || id;
    ...
    const recipes = recipeRows.map(r => {
      ...
      const folder = CorpusQaService.cell_(r[4]);
      return { id, name: nameOf(r, id), project, folder, path: resolve(project, folder) };
    }).sort(byName);
    const tables = tableRows.map(r => {
      const id = String(r[0]);
      return { id, name: nameOf(r, id) };
    }).sort(byName);
...
/**
 * The entity browser's feed: recipes as {id, name, project, folder, path} (***UPDATED*** O12)
 * and data tables as {id, name}. Throws on a failed read so google.script.run failure handlers
 * render the cause — loudly, by design.
 */
