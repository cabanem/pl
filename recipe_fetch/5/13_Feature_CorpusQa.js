 ...
 *
 *   ***UPDATED*** Freshness + entity-browser slice adds two read-only endpoints with OPPOSITE
 *   error laws, both recorded in the handoff: status() feeds the web app's ambient freshness
 *   chip and starter chips (NEVER throws — an absent chip claims nothing), and entities() feeds
 *   the entity browser (THROWS — an error must never impersonate an empty estate).
 *
...
class CorpusQaService {
  ...
  static status() {
    ...
    } catch (e) { AppLog.verbose(`Corpus status: store leg degraded: ${(e && e.message) || e}`); }
    try {
      out.recipes = new ChangeLedgerRunner().readRows_("RECIPES").filter(r => r && r[0]).length;
    } catch (e) { AppLog.verbose(`Corpus status: recipe-count leg degraded: ${(e && e.message) || e}`); }
    try {
    ...
    } catch (e) { AppLog.verbose(`Corpus status: starters leg degraded: ${(e && e.message) || e}`); }
    return out;
  }

  /**
   * ***UPDATED*** Entity-browser feed: id+name pairs from RECIPES and TABLES through the ledger's
   * key-resolving read seam — the vocabulary users need before they can ask about things they
   * can't name. Sorted by name (case-insensitive), blank-id rows dropped, a nameless row falls
   * back to its id. Read-only, no new scopes, no schema change.
   *
   ...
   * estate. A genuinely empty estate returns empty arrays: that is a state, not an error.
   * @returns {{recipes:Array<{id:string,name:string}>, tables:Array<{id:string,name:string}>}}
   */
  static entities() {
    const led = new ChangeLedgerRunner();
    const pairs = rows => (rows || [])
      ...
      .map(r => {
        const id = String(r[0]);
        return { id, name: String(r[1] == null ? "" : r[1]).trim() || id };
      })
      .sort((a, b) => a.name.toLowerCase().localeCompare(b.name.toLowerCase()));
    return { recipes: pairs(led.readRows_("RECIPES")), tables: pairs(led.readRows_("TABLES")) };
  }
...
/**
 * ***UPDATED*** The entity browser's feed (recipes + data tables as id/name pairs). Throws on
 * a failed read so google.script.run failure handlers render the cause — loudly, by design.
 */
