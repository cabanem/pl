 ...
 *     status + entity feed (***UPDATED***), config + cron wiring. Sheet reads/appends ride the withLedgerBackend_ in-memory backend; Drive and
...
function registerCorpusHermeticTests(runner) {
  ...
  runner.add("[hermetic] getCorpusEntities serves sorted id+name pairs from RECIPES and TABLES through the ledger seam", () => {
    ...
    withLedgerBackend_(fake, () => {
      ...
      Assert.equal(e.recipes[2].name, "PRV-01 Root", "id+name pairs, nothing more");
