...
# Ghost 1: the ENV VAR overrides gcloud config entirely — config unset cannot
# touch it. Clear it for this process (your parent shell is unaffected).
if [ -n "${CLOUDSDK_AUTH_IMPERSONATE_SERVICE_ACCOUNT:-}" ]; then
  echo "[0a] clearing CLOUDSDK_AUTH_IMPERSONATE_SERVICE_ACCOUNT (was: ${CLOUDSDK_AUTH_IMPERSONATE_SERVICE_ACCOUNT}) for this run"
  unset CLOUDSDK_AUTH_IMPERSONATE_SERVICE_ACCOUNT
fi
# Ghost 2: a DEPLOY_SA exported by an earlier attempt silently re-enters
# choreography mode. Make the mode a visible choice, never an inheritance:
if [ -n "${DEPLOY_SA}" ]; then
  echo "[0a] NOTE: choreography mode ACTIVE — DEPLOY_SA=${DEPLOY_SA}"
  echo "     If you meant to deploy as yourself:  unset DEPLOY_SA  and re-run."
fi
