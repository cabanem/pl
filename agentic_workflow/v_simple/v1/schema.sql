...
-- connection). `resolved`=0 marks the known export degradation where
-- flow_id collapses to a bare numeric string — keep these visible rather
-- than dropping them; an unresolved edge is itself a fact worth surfacing.
CREATE TABLE edges (
