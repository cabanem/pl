...
class CorpusQaService {
  ...
  static ask(question, ctx = null) {
    ...
    // ***UPDATED*** Duration: time the Gemini call so the sync-vs-ticket decision (WEBAPP_PLAN §5)
    // is made by the real p95, not a guess. O7: a failed call still persists below -- error rows are
    // the loudest latency signal. Honest limit: a hard platform kill (~6 min) runs no code after it,
    ...
    // ***UPDATED*** O7: the row above is the durable trace; the throw keeps the client contract --
  ...
  /**
   * THE seam. v1: the whole digest, one Drive read (~instant at 58 recipes -- a CacheService layer
   ...
   * Answers are capped at the sheet cell limit with a loud suffix -- never silently clipped.
   * ***UPDATED*** meta carries the two appended columns: { durationMs, status } -- ms of the Gemini
...
/**
 * ***UPDATED*** Editor probe for the history read path -- the deterministic bisect step whenever a
