...
class SchemaDef {
  ...
  static get HEADERS() {
    return {
      ...
      // ***UPDATED*** Web-app round: Duration ms (the ticket-lane decision reads real p95 from here,
      // WEBAPP_PLAN §5) and Status ("ok" | "error" — O7: failed asks persist as visible rows). Appended
      // at the END so every existing reader's indexes stay valid; run migrateQaLogHeaderV2 once to
      // widen a live tab's header (appendRows_ only writes headers when it creates a tab).
