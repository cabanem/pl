...
function registerLedgerIntegrationTests(runner) {
  ...
  runner.add("[integration] ledger lifecycle: baseline -> quiet -> code change -> edge removal", () => {
    ...
    withLedgerBackend_(fakeSheets, (journal) => {
      ...
      // ***UPDATED*** shape, not library string — per this suite's own tier-2 charter (see 50's header),
      // exact library output belongs in COMMENTED checks until confirmed against the live library.
      // The load-bearing halves both stay asserted: 999 is REPORTED here, and never ORDERED (the
      // edgeState deepEqual above). On failure, the actual rows print for one-look diagnosis.
      const weakReported = ambiguity.some(s => s.includes("999"));
      ...
      // --- Exact finding-code check (OrderLib-version-dependent). Enable once confirmed live:
      // Assert.ok(ambiguity.some(s => s.includes("WEAK_EDGE") && s.includes("999")), "decoy carries the WEAK_EDGE code");
