 ...
 *     content, response shaping, QA_LOG schema + row shaping + history ordering, config + cron
 *     wiring. Sheet reads/appends ride the withLedgerBackend_ in-memory backend; Drive and
...
function registerCorpusHermeticTests(runner) {
  ...
  // --- QA_LOG persistence ---------------------------------------------------------------
  runner.add("[hermetic] QA_LOG schema present: key resolves, eight columns, flags where the code expects them", () => {
    Assert.equal(SchemaDef.SHEETS.QA_LOG, "Output_QA_Log", "key -> tab name");
    const h = SchemaDef.HEADERS.QA_LOG;
    Assert.equal(h.length, 8, "eight columns");
  ...
  runner.add("[hermetic] recent() returns newest-first, caps the window, and maps the refusal flag to a boolean", () => {
    const fake = new CorpusFakeSheetService();
    ...
    withLedgerBackend_(fake, () => {
      ...
      Assert.equal(out[0].question, "q3", "newest first");
      Assert.equal(out[1].question, "q2", "…then the one before it");
      Assert.equal(out[1].refused, true, "'yes' string mapped to a boolean");
      Assert.equal(out[0].refused, false, "empty string mapped to false");
      Assert.equal(CorpusQaService.recent(50).length, 3, "asking for more than exists returns all");
    });
  });
