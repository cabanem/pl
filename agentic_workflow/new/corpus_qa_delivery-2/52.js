 ...
 * @description
 *   Test coverage for the change-ledger wiring (11_Feature_ChangeLedger, AiGate, 12_Cron, the AiAnalysisRunner
 *   merge/stamp changes, and the GeminiService evidence contract). Same two-tier scheme as 50_Tests_Integration:
 *
 *   TIER 1 — HERMETIC  (registerLedgerHermeticTests)
 *     Pure logic in THIS project: diff classification, fingerprinting, the change-window helper, gate math,
 *     schema-drift guards, cron wiring. Sheet-touching readers are prototype-patched to in-memory backends,
 *     so NOTHING here reads or writes a real tab. Deterministic, fast.
 *
 *   TIER 2 — INTEGRATION  (registerLedgerIntegrationTests)
 *     Crosses into the bound libraries (WorkatoGraphLib, OrderLib, GeminiLib) with FakeWorkatoClient seeds (includes
 *     the full ledger lifecycle (baseline → quiet night → code change → edge removal) and the gated AI runner's
 *     merge / Source FP stamping / error-row alignment. Skips cleanly when a library isn't bound, matching the 50_Tests
 *     philosophy. Still no real tabs and no live network: fakes all the way down.
 *
...
function registerLedgerIntegrationTests(runner) {
  ...
  runner.add("[integration] ledger lifecycle: baseline -> quiet -> code change -> edge removal", () => {
    ...
    withLedgerBackend_(fakeSheets, (journal) => {
      ...
      // ***UPDATED*** 2026-08-27: the live library's WEAK_EDGE detail names the key and step but not the
      // observed value ("999"), so matching on the value coupled this test to a message format the
      // library never promised. Match on what is semantically stable instead: the WEAK_EDGE code
      // (now confirmed live) plus the fixture's key and step names. Behavior under test is unchanged --
      // the decoy is reported as a finding and never ordered (the EDGE_STATE assert above proves the latter).
      const weakReported = ambiguity.some(s =>
        s.includes("WEAK_EDGE") && s.includes("recipe_id") && s.includes("Log reference"));
      if (!weakReported) console.error("AMBIGUITY rows for diagnosis: " + JSON.stringify(ambiguity));
      Assert.ok(weakReported, "id-shaped key on a non-call step reported in AMBIGUITY, never ordered");
  ...
  runner.add("[integration] AiAnalysisRunner merges kept rows, stamps Source FP, aligns error rows", () => {
    ...
    withTestConfig({ VERTEX: { GOOGLE_CLOUD_PROJECT_ID: "test-project" } }, () => {
      ...
      withLedgerBackend_(fakeSheets, () => {
        ...
        const canned = {
          objective: "Test objective", trigger: "Manual", high_level_flow: ["Does a thing (step 1)"],
  ...
  runner.add("[integration] evidence contract present in the structured prompt", () => {
    withTestConfig({ VERTEX: { GOOGLE_CLOUD_PROJECT_ID: "test-project" } }, () => {
      ...
      const prompt = svc._buildStructuredPrompt({
        name: "R", description: "", trigger_app: "",
