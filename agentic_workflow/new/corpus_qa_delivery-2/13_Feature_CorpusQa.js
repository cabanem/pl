...
class CorpusQaService {
  ...
  static ask(question, ctx = null) {
    ...
    const parsed = new GeminiService().answerFromCorpus(digest, q);
    const result = CorpusQaService._shapeAnswer_(parsed, q, asOf);
    ...
    // ***UPDATED*** Durable Q&A record. System_Logs is audit-only AND pruned (Maintenance keeps the
  ...
  /**
   * ***UPDATED*** Newest-first Q&A history from the QA_LOG tab, shaped for the sidebar and the
  ...
  /**
   * ***UPDATED*** One QA_LOG row in HEADERS.QA_LOG order, pure and hermetically testable.
   * Answers are capped at the sheet cell limit with a loud suffix — never silently clipped.
   * @private
   */
  static _qaLogRow_(result, who, nowIso, charLimit) {
...
/**
 * ***UPDATED*** History source for the sidebar (and the future web app): the newest `limit`
