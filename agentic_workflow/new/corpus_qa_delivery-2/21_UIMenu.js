...
class UserInterfaceService {
  ...
  createMenu() {
    ...
    if (!isAdv) {
      ...
      root.addSeparator();
      root.addItem("Ask the corpus (Q&A)", "showCorpusSidebar"); // ***UPDATED*** sidebar is the primary ask surface: rendered answers + history
    } else {
      ...
      root.addSubMenu( // ***UPDATED*** Corpus Q&A: sidebar is primary; the quick dialog and digest rebuild stay reachable
        this.ui.createMenu("Corpus Q&A")
          .addItem("Open Q&A sidebar", "showCorpusSidebar")
          .addItem("Quick ask (dialog)", "askCorpusPrompt")
  ...
  showConfiguration() {
    ...
    const vertexStatus =
      ...
      user['GOOGLE_CLOUD_PROJECT_ID'] ? `${user['GOOGLE_CLOUD_PROJECT_ID']} (User)` : // ***UPDATED*** bug fix: unquoted key threw ReferenceError whenever only the user-scope property was set
      "NOT SET";
