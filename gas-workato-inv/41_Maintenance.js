 ...
 *
 *   Why each exists:
 *   - Inventory/analysis sheets are OVERWRITE-on-sync (SheetService.write clears
 *     first), so they never accumulate stale rows. "Reset" is for handoff,
 *     switching workspaces/tokens, or decommissioning — not routine hygiene —
 *     and is reversible, since a sync regenerates everything.
 *   - System_Logs is APPEND-only, so it grows without bound. Prune keeps the
 *     most recent N rows.
 *   - The Drive debug folder gets a new timestamped file per debug/doc run, so
 *     it grows too. Purge moves files older than N days to Trash (recoverable),
...
class MaintenanceService {
   ...
   * Reuses SheetService.write so headers are re-created and formatted exactly as
   * a sync would. Leaves Input_Requests, System_Logs, AI/process-map outputs,
   * and the dashboard untouched. Reversible via a workspace sync.
   */
  ...
  // --- INTERNALS ---------------------------------------------------------------------------------------
  /**
   * Resolve the debug folder WITHOUT creating it (unlike DriveService, which
   * creates + caches). Returns null if it doesn't exist — a purge shouldn't
   * conjure an empty folder.
   * @private
   */
